...
public class ToDoTasksController : BaseApiController
{
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<IEnumerable<ToDoTaskDto>>> GetById([FromRoute] int id)
    {
        return HandleResult(await Mediator.Send(new GetByIdToDoTaskQuery(id)));
    }

    [HttpDelete("{id:int}")]
    public async Task<ActionResult<IEnumerable<ToDoTaskDto>>> Delete([FromRoute] int id)
    {
